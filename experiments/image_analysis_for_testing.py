...
class SpeedFinder():
    ...
    def get_speed(self, im):
        ...
        try:
            all_clusters, clusters, kmap, _ = self.get_color_clusters(smaller_im, n_clusters=4) # TODO: this one takes a while
        except:
        ...
        for i, cluster in enumerate(filtered_clusters):
            ...
            cluster_edges = canny(self.cluster_to_img(cluster, smaller_im_shape), sigma=0.7,
              low_threshold=0.55, high_threshold=0.9)
            tested_angles = np.linspace(-np.pi / 2, np.pi / 2, 360, endpoint=False) #new!
            h, theta, d = hough_line(cluster_edges, theta=tested_angles) #improved!
            lines = list(zip(*hough_line_peaks(h, theta, d)))
        ...
        self.show_many_clusters(line_clusters, smaller_im_shape)
